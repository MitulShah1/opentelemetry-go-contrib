...
package otelfiber // import "go.opentelemetry.io/contrib/instrumentation/github.com/otelfiber/fiber/otelfiber"
...
type config struct {
	...
	FiberMetricAttributesFn func(*fiber.Ctx) []attribute.KeyValue
}
...
// FiberFilter filters an [net/http.Request] based on content of a [fiber.Ctx].
type FiberFilter func(*fiber.Ctx) bool
...
// FiberMetricAttributeFn is used to extract additional attributes from the fiber.Ctx
// and return them as a slice of attribute.KeyValue.
type FiberMetricAttributeFn func(*fiber.Ctx) []attribute.KeyValue
...
// WithFiberFilter adds a gin filter to the list of filters used by the handler.
func WithFiberFilter(f ...FiberFilter) Option {
	return optionFunc(func(c *config) {
		c.FiberFilter = append(c.FiberFilter, f...)
	})
}
...
func WithMetricAttributesFn(metricAttributesFn MetricAttributeFn) Option {
	return optionFunc(func(c *config) {
		c.MetricAttributeFn = metricAttributesFn
	})
}
...
func WithFiberMetricAttributeFn(f FiberMetricAttributeFn) Option {
	return optionFunc(func(c *config) {
		c.FiberMetricAttributesFn = f
	})
}
